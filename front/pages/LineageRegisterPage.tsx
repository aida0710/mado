  ...
  return (
    <form className="manual-form" onSubmit={submit}>
      <section className="manual-section">
        <header><span>01</span><h3>Dataset</h3></header>
        <div className="manual-segmented" role="group" aria-label="Datasetの登録方法">
          <button type="button" data-active={target === 'new' || undefined} onClick={() => { setTarget('new'); setDataset(null) }}>新しく作る</button>
          <button type="button" data-active={target === 'existing' || undefined} onClick={() => setTarget('existing')}>既存に版を追加</button>
        </div>
        {target === 'existing' ? <DatasetPicker value={dataset} onChange={setDataset} /> : (
          <div className="manual-grid manual-grid--2">
            ...
            <label className="manual-field"><span>Dataset key</span><input name="datasetKey" required placeholder="podcast/ja/raw" /></label>
            <label className="manual-field"><span>Namespace</span><input name="namespace" required placeholder="mdx-speech" /></label>
            <label className="manual-field"><span>Name</span><input name="name" required placeholder="podcast/ja/raw" /></label>
            <label className="manual-field"><span>Media type</span><input name="mediaType" placeholder="audio" /></label>
            <label className="manual-field"><span>Owner</span><input name="owner" /></label>
      ...
      <section className="manual-section">
        <header><span>02</span><h3>Version</h3></header>
        <div className="manual-grid manual-grid--2">
          <label className="manual-field"><span>Version</span><input name="version" required placeholder="2026-08-27" /></label>
          <label className="manual-field"><span>Content hash</span><input name="contentHash" placeholder="sha256:..." /></label>
          <label className="manual-field"><span>Manifest URI</span><input name="manifestUri" placeholder="s3://.../manifest.jsonl" /></label>
          <label className="manual-field"><span>Manifest hash</span><input name="manifestHash" placeholder="sha256:..." /></label>
          <label className="manual-field manual-field--wide"><span>Schema URI</span><input name="schemaUri" /></label>
        </div>
      </section>
      ...
      <section className="manual-section">
        ...
        <div className="manual-options">
          ...
          <label className="manual-check"><input type="checkbox" checked={withProcessing} onChange={event => setWithProcessing(event.target.checked)} /> この版を作った処理を記録する</label>
        </div>
        {withSource && (
          <div className="manual-subsection">
            <div className="manual-grid manual-grid--2">
              <label className="manual-field"><span>Source key</span><input name="sourceKey" required placeholder="vendor/corpus-name" /></label>
              <label className="manual-field"><span>種類</span><select name="sourceKind" defaultValue="purchased"><option value="purchased">購入</option><option value="crawled">収集</option><option value="provided">提供</option><option value="database">Database</option><option value="generated">生成</option><option value="other">その他</option></select></label>
              ...
              <label className="manual-field"><span>Vendor</span><input name="vendor" /></label>
              <label className="manual-field"><span>Product</span><input name="product" /></label>
              <label className="manual-field"><span>License参照</span><input name="licenseRef" /></label>
              <label className="manual-field"><span>Contract参照</span><input name="contractRef" /></label>
            </div>
          </div>
        )}
        {withProcessing && (
          <div className="manual-subsection">
            <VersionListPicker values={inputs} onChange={setInputs} label="入力Version" />
            <div className="manual-grid manual-grid--2">
              <label className="manual-field"><span>処理名</span><input name="transformationName" required placeholder="NeMo Curator 品質フィルタ" /></label>
              <label className="manual-field"><span>Transformation key</span><input name="transformationKey" required placeholder="nemo-curator-quality-filter" /></label>
              <label className="manual-field"><span>Job namespace</span><input name="jobNamespace" required placeholder="mdx-speech" /></label>
              <label className="manual-field"><span>Job name</span><input name="jobName" required placeholder="nemo-curator-quality-filter" /></label>
              <label className="manual-field"><span>Repository</span><input name="codeRepository" placeholder="https://github.com/..." /></label>
              <label className="manual-field"><span>Code ref</span><input name="defaultCodeRef" placeholder="commit / tag" /></label>
              <label className="manual-field"><span>Git SHA</span><input name="gitSha" /></label>
              <label className="manual-field"><span>Container digest</span><input name="containerDigest" /></label>
              <label className="manual-field"><span>Config URI</span><input name="configUri" /></label>
              <label className="manual-field"><span>Config hash</span><input name="configHash" /></label>
      ...
      <section className="manual-section manual-review">
        ...
        <p>保存するとRegistryへ確定登録され、処理履歴がある場合はOpenLineage projectionへ送られます。</p>
        {error && <p className="error" role="alert">{error}</p>}
        {resultVersionId && <ResultNotice message="Dataset Versionを登録しました。" to={`/lineage?mode=versions&versionId=${encodeURIComponent(resultVersionId)}`} />}
...
function LocationRegistrationForm({ connections }: { connections: Connection[] }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    if (!version) return setError('Dataset Versionを選択してください。')
    const form = new FormData(event.currentTarget)
  ...
  return (
    <form className="manual-form" onSubmit={submit}>
      <div className="manual-guidance"><strong>同じ内容を別の場所へコピーした場合はこちら</strong><span>新しいVersionや処理履歴は作らず、同じ版へ保存場所だけを追加します。</span></div>
      <section className="manual-section"><header><span>01</span><h3>Version</h3></header><VersionPicker value={version} onChange={setVersion} /></section>
...
function RunRegistrationForm() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (inputs.some(input => outputs.some(output => output.id === input.id))) return setError('同じVersionを入力と出力の両方には指定できません。コピーは「保存場所」を使ってください。')
    const form = new FormData(event.currentTarget)
  ...
  return (
    <form className="manual-form" onSubmit={submit}>
      ...
      <section className="manual-section"><header><span>01</span><h3>入出力</h3></header>
        <div className="manual-grid manual-grid--2 manual-grid--top"><VersionListPicker values={inputs} onChange={setInputs} label="入力Version" /><VersionListPicker values={outputs} onChange={setOutputs} label="出力Version" /></div>
      </section>
      <section className="manual-section"><header><span>02</span><h3>処理</h3></header>
        <div className="manual-grid manual-grid--2">
          ...
          <label className="manual-field"><span>Transformation key</span><input name="transformationKey" required /></label>
          <label className="manual-field"><span>Job namespace</span><input name="jobNamespace" required /></label>
          <label className="manual-field"><span>Job name</span><input name="jobName" required /></label>
          <label className="manual-field"><span>Repository</span><input name="codeRepository" /></label>
          <label className="manual-field"><span>Code ref</span><input name="defaultCodeRef" /></label>
          <label className="manual-field"><span>Git SHA</span><input name="gitSha" /></label>
          <label className="manual-field"><span>Container digest</span><input name="containerDigest" /></label>
          <label className="manual-field"><span>Config URI</span><input name="configUri" /></label>
          <label className="manual-field"><span>Config hash</span><input name="configHash" /></label>
...
export default function LineageRegisterPage() {
  ...
  if (!allowed) {
    return <section><header className="page-head"><h2>Lineage</h2></header><p className="error">手動登録の権限がありません。</p></section>
  }

  return (
    <section className="manual-lineage-page">
      <header className="page-head"><h2>Lineage</h2><Link className="ghost" to="/lineage">一覧に戻る</Link></header>
      <nav className="manual-kind-nav" aria-label="登録種別">
        <button type="button" data-active={kind === 'dataset' || undefined} onClick={() => setKind('dataset')}>Dataset</button>
