...
- **容量メトリクス** — 明示的に追跡したバケットの容量・オブジェクト数を定期取得し、Rechartsで増減を確認
...
バケットを切り替えられる容量履歴画面を開けます。定期計測は初期状態で無効です。
`connections:manage`権限を持つUserが6時間〜7日の周期を選んで有効化し、
`jobs:operate`権限を持つUserは **「今すぐ計測」** を実行できます。完全なバケット全体走査だけが
履歴になり、途中終了した値はグラフへ混ぜません。
