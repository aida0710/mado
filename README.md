...
途中終了した値はグラフへ混ぜません。
コネクション編集の **「バケットのメトリクス集計を許可する」** は既定で有効です。
無効にすると強制計測を止め、定期計測の有効化と周期設定も操作できなくなります。
走査の1ページ件数はコネクションごとに100／250／500／1,000件から選べます。
既定は1,000件で、S3互換実装の一覧応答が遅い場合だけ小さくします。
