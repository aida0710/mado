---
title: Web Dashboard 設計（スパコン + DDN 製オブジェクトストレージ）
date: 2026-04-30
status: draft
authors: aida
---
...
- **S3 ページ**: DDN 製オブジェクトストレージ上の S3 バケットをブラウズし、各ディレクトリに README.md を読み書きでき、テキスト/画像/音声/tar 系の中身プレビューができる。
...
- DDN 製オブジェクトストレージ上の S3 バケット構造を視覚的にブラウズできる。
...
- ダッシュボードに到達できる人 = DDN 製オブジェクトストレージ上の全バケットを閲覧できる人。これは設計上の仕様であり、防御は LAN 境界で行う。
...
ページ切替は上部ヘッダのタブ（`[スパコン]` `[S3]`）。サイドバーは作らない。
...
| `S3_ENDPOINT` | — | S3 互換ストレージのエンドポイント URL |
